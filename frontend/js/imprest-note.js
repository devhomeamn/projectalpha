    ...
    .map(
      ...
      <button type="button" class="imp-code-search-item" data-add-code-id="${row.id}">
        <span class="meta">
          ...
          <span class="line2">Budget: ${formatMoney(row.budget)}</span>
        </span>
...
async function loadBudgetCodes() {
  ...
  state.availableCodes = Array.isArray(out.data) ? out.data : [];
  renderAddCodeList();
}
...
function recalcTotalsAndPaint() {
  ...
  byId("impTotalBudget").textContent = formatMoney(note.total_budget);
  byId("impTotalPrevious").textContent = formatMoney(note.total_previous_expense);
  byId("impTotalClaim").textContent = formatMoney(note.total_current_claim);
  byId("impTotalRemaining").textContent = formatMoney(note.total_remaining);
}
...
function renderRows(note) {
  ...
  tbody.innerHTML = note.items
    .map((item, idx) => {
      const claim = toNumber(item.current_claim).toFixed(2);
      const codeId = toPositiveInt(item.financial_code_id);
      return `
      <tr id="imp-row-code-${codeId}">
        ...
        <td class="imp-right">${formatMoney(item.budget_amount)}</td>
        <td class="imp-right">${formatMoney(item.previous_expense)}</td>
        <td class="imp-right">
          <input
        ...
        <td class="imp-right imp-row-remaining">${formatMoney(item.remaining_balance)}</td>
        <td>
          ${
            editable
...
function buildPrintHtml(payload) {
  ...
  const paragraphText = `\u0989\u09aa\u09b0\u09cd\u09af\u09c1\u0995\u09cd\u09a4 \u09ac\u09bf\u09b7\u09af\u09bc\u09c7 ${note.period_start || "-"} \u09b9\u09a4\u09c7 ${note.period_end || "-"} \u09aa\u09b0\u09cd\u09af\u09a8\u09cd\u09a4 \u09b8\u09ae\u09af\u09bc\u09c7\u09b0 \u09ac\u09cd\u09af\u09af\u09bc \u09a8\u09bf\u09b0\u09cd\u09ac\u09be\u09b9\u09c7\u09b0 \u09a8\u09bf\u09ae\u09bf\u09a4\u09cd\u09a4\u09c7 = ${formatMoneyBn(
    totalClaim
  )}/- (${amountWords}) \u099f\u09be\u0995\u09be\u09b0 \u0986\u09b0\u09cd\u09a5\u09bf\u0995 \u09a6\u09be\u09ac\u09c0 \u09aa\u09be\u0993\u09af\u09bc\u09be \u0997\u09c7\u099b\u09c7\u0964`;
  const footerText = `\u098f\u09a4\u09a6\u09cd\u09ac\u09bf\u09b7\u09af\u09bc\u09c7, \u09b8\u0982\u09b6\u09cd\u09b2\u09bf\u09b7\u09cd\u099f \u0998\u09be\u0981\u099f\u09bf \u0995\u09b0\u09cd\u09a4\u09c3\u0995 \u09a6\u09be\u09ac\u09c0\u0995\u09c3\u09a4 = ${formatMoneyBn(
    totalClaim
