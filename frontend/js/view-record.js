...
function renderTable(records) {
  ...
  records.forEach((rec) => {
    ...
    const rackName = rec.Rack ? rec.Rack.name : "-"; // ✅ fixed line
...
document.getElementById("searchInput").addEventListener("input", (e) => {
  ...
  const filtered = allRecords.filter(
    (r) =>
      ...
      (r.Rack && r.Rack.name.toLowerCase().includes(term)) // ✅ added this
  );
...
function exportToCSV() {
  ...
  const rows = allRecords.map((r) => [
    ...
    r.Rack ? r.Rack.name : "", // ✅ fixed here too
    r.added_by,
...
function showDetails(rec) {
  ...
  const rackName = rec.Rack ? rec.Rack.name : "-"; // ✅ fixed here
